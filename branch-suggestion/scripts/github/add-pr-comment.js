...
async function addOrUpdateComment(
  ...
  try {
    ...
    if (existingComment) {
      ...
      const updated = await updatePRComment(
        owner,
        repo,
        existingComment.id,
        commentBody,
    ...
    throw new Error(
      `Failed to add/update comment on PR #${prNumber}: ${error.message}`,
    );
  }
}
...
async function main() {
  ...
  if (args.length < 2) {
    console.log(
      "Usage: node add-pr-comment.js <owner/repo> <pr-number> [<comment-body>] [options]",
    );
    console.log("\nOptions:");
    console.log("  --file <path>       Read comment body from file");
    console.log(
      "  --marker <marker>   Custom marker (default: <!-- branch-suggestions -->)",
    );
    console.log("\nExamples:");
    console.log("  # Direct comment text");
    console.log(
      '  node add-pr-comment.js TykTechnologies/tyk 123 "## Suggested branches\\n- release-5.8"',
    );
    console.log("");
    console.log("  # From file");
    console.log(
      "  node add-pr-comment.js TykTechnologies/tyk 123 --file /tmp/comment.md",
    );
    console.log("");
    console.log("  # Custom marker");
    console.log(
      '  node add-pr-comment.js TykTechnologies/tyk 123 "Comment" --marker "<!-- custom -->"',
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (!commentBody || commentBody.trim() === "") {
      throw new Error(
        "Comment body cannot be empty. Use --file <path> to read from file.",
      );
    }
...
export { addOrUpdateComment, parseRepo, COMMENT_MARKER };
